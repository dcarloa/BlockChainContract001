...
async function loadSmartSettlements() {
    try {
        ...
        const payLabel = t('app.settlements.actions.pay') || 'Pay';
        const remindLabel = t('app.settlements.actions.remind') || 'Remind';
        ...
        settlements.forEach((settlement, index) => {
            ...
            const truncate = (str, len = 15) => str.length > len ? str.substring(0, len) + '...' : str;
            
            // Check if current user is the debtor (can pay)
            const currentUserId = firebase.auth().currentUser?.uid;
                    ...
                    <div class="settlement-content">
                        <div class="settlement-flow">
                            ...
                            <div class="settlement-to">
                                <div class="settlement-avatar" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);">${toName.charAt(0).toUpperCase()}</div>
                                <div class="settlement-name" title="${toName}">${truncate(toName)}</div>
                            </div>
                        </div>
                        <div class="settlement-actions">
                            <button class="settlement-pay-btn" onclick="openPaymentMethodModal(${index})" title="Record payment">
                                <span>💳</span>
                                <span>${payLabel}</span>
                            </button>
                            ${isCreditor ? `
                            <button class="settlement-remind-btn" onclick="sendQuickReminder(${index})" title="Send reminder">
                                🔔
                            </button>
                            ` : ''}
                        </div>
...
// ============================================
// PAYMENT METHOD SELECTION (Phase 2)
// ============================================

// Current settlement being processed
let pendingSettlementIndex = null;
let selectedPaymentMethod = null;

/**
 * Open payment method modal for a specific settlement
 */
function openPaymentMethodModal(index) {
    ...
    // Show quick actions
    const quickActions = document.getElementById('paymentQuickActions');
    if (quickActions) quickActions.style.display = 'flex';
    
    // Populate payment info summary
    const fromMember = currentFund?.members?.[settlement.from];
    const toMember = currentFund?.members?.[settlement.to];
    const fromName = fromMember?.name || fromMember?.email || 'Unknown';
    const toName = toMember?.name || toMember?.email || 'Unknown';
    ...
    const summaryHtml = `
        <div class="payment-info-from">
            <div class="payment-info-avatar">${fromName.charAt(0).toUpperCase()}</div>
            <div class="payment-info-name">${truncateName(fromName, 12)}</div>
        </div>
        <div class="payment-info-amount">
            <div class="payment-info-arrow">→</div>
            <div class="payment-info-value">$${settlement.amount.toFixed(2)}</div>
        </div>
        <div class="payment-info-to">
            <div class="payment-info-avatar" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);">${toName.charAt(0).toUpperCase()}</div>
            <div class="payment-info-name">${truncateName(toName, 12)}</div>
        </div>
    `;
    
    const summaryEl = document.getElementById('paymentInfoSummary');
...
/**
 * Confirm settlement with selected payment method
 */
async function confirmSettlementWithMethod() {
    if (pendingSettlementIndex === null || !selectedPaymentMethod) {
        showToast('Please select a payment method', 'error');
        return;
    }
    
    const settlement = currentSettlements[pendingSettlementIndex];
    if (!settlement) {
        showToast('Settlement not found', 'error');
        return;
    }
    ...
    try {
        ...
        const settlementInfo = {
            ...
            notes: `Paid via ${methodLabels[selectedPaymentMethod]} on ${new Date().toLocaleDateString()}`
        };
        ...
        // Animate settlement in list
        const settlementEl = document.getElementById(`settlement-${pendingSettlementIndex}`);
        if (settlementEl) {
            settlementEl.classList.add('settlement-completed');
            
            // Add method badge
            const methodBadge = document.createElement('div');
            methodBadge.className = 'settlement-method-badge';
            methodBadge.innerHTML = `<span>${getPaymentMethodIcon(selectedPaymentMethod)}</span> ${methodLabels[selectedPaymentMethod]}`;
            settlementEl.querySelector('.settlement-content')?.appendChild(methodBadge);
        }
        
        showToast(`Payment recorded via ${methodLabels[selectedPaymentMethod]}! ✅`, 'success');
        ...
        // Track analytics
        if (typeof gtag === 'function') {
            gtag('event', 'settlement_completed', {
                'event_category': 'settlements',
...
async function copyPaymentInfo() {
    if (pendingSettlementIndex === null) return;
    ...
    const fromName = fromMember?.name || fromMember?.email || 'Unknown';
    const toName = toMember?.name || toMember?.email || 'Unknown';
    const toEmail = toMember?.email || '';
    const toPhone = toMember?.phone || toMember?.paymentInfo?.phone || '';
    
    let paymentInfo = `💰 Payment Request\n`;
    paymentInfo += `From: ${fromName}\n`;
    paymentInfo += `To: ${toName}\n`;
    paymentInfo += `Amount: $${settlement.amount.toFixed(2)}\n`;
    
    if (toEmail) paymentInfo += `Email: ${toEmail}\n`;
    if (toPhone) paymentInfo += `Phone: ${toPhone}\n`;
    ...
    try {
        ...
        const copyBtn = document.getElementById('copyPaymentInfoBtn');
        if (copyBtn) {
            copyBtn.classList.add('copied');
            copyBtn.querySelector('span:last-child').textContent = t('app.settlements.actions.copied') || 'Copied!';
            
            setTimeout(() => {
                copyBtn.classList.remove('copied');
                copyBtn.querySelector('span:last-child').textContent = t('app.settlements.actions.copyInfo') || 'Copy Payment Info';
            }, 2000);
        }
        
        showToast('Payment info copied! 📋', 'success');
...
async function sendPaymentReminder() {
    ...
    try {
        ...
        if (window.FirebaseConfig && window.FirebaseConfig.sendPushNotification) {
            await window.FirebaseConfig.sendPushNotification({
                title: '💰 Payment Reminder',
                body: `Hey! Just a friendly reminder that you owe ${toName} $${settlement.amount.toFixed(2)}`,
                targetUserId: settlement.from,
                data: {
                    type: 'payment_reminder',
                    groupId: currentFund.fundId,
                    amount: settlement.amount
                }
            });
            ...
            if (navigator.share) {
                await navigator.share({
                    title: 'Payment Reminder - Ant Pool',
                    text: `Hey ${fromName}! Just a reminder that you owe ${toName} $${settlement.amount.toFixed(2)} 💰`,
                    url: window.location.href
                });
            } else {
                // Fallback: copy reminder text
                const reminderText = `Hey ${fromName}! Just a reminder that you owe ${toName} $${settlement.amount.toFixed(2)} 💰\n\n— Sent via Ant Pool`;
...
async function sendQuickReminder(index) {
    ...
    try {
        ...
        if (navigator.share) {
            ...
            showToast(`Reminder shared! 🔔`, 'success');
        } else {
            // Fallback: copy reminder text
            const reminderText = `Hey ${fromName}! Just a reminder that you owe ${toName} $${settlement.amount.toFixed(2)} 💰\n\n— Sent via Ant Pool`;
            await navigator.clipboard.writeText(reminderText);
